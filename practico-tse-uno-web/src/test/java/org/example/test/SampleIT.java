...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "practico-tse-uno-ear.ear");

        // Current directory is the root of the "practico-tse-uno-web" project. Go up one level
        // and enter the "practico-tse-uno-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../practico-tse-uno-ejb/target/practico-tse-uno-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "practico-tse-uno-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/practico-tse-uno-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "practico-tse-uno-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
